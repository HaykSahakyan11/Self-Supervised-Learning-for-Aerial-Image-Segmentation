...
def load_dino_deit_seg_model(seg_ckpt, n_classes, img_sz, patch_sz, bck=None):
...
def export_dino_deit_split_logits(split: str, batch: int, seg_ckpt, out_dir: str = None, data_root: str = None):
    ...
    model = load_dino_deit_seg_model(seg_ckpt, inf_dataset.num_classes,
                                   config.image_size, config.patch_size, bck=bck)
...
if __name__ == "__main__":
    ...
    for split in ['train', 'val']:
        ...
        save_dir = config.UAVID_patch_inf['dino_deit']['4']['no_overlap'][split]
